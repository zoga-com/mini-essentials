package ru.zoga_com.essentials.commands.nms;
...
public class CommandPing_1_17 implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if(cmd.getName().equalsIgnoreCase("ping")) {
          if(config.getString("lang").equalsIgnoreCase("en")) {
              p.sendMessage(langEn.getString("ping.before") + getPing(p) + langEn.getString("ping.after"));
          }
          if(config.getString("lang").equalsIgnoreCase("ru")) {
              p.sendMessage(langRu.getString("ping.before") + getPing(p) + langRu.getString("ping.after"));
          }
    }
    return true;
    }
